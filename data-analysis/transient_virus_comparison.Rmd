...
```{r viral comparisons, echo=FALSE}
...
### all viruses
```{r infant infection comparisons, echo=FALSE}
...
### CMV only
```{r infant infection comparisons cmv only, echo=FALSE}
...
```{r bf analysis, echo = F, warning = F}
...
select(covariate_rates, var, value, everything()) %>% ungroup() %>% kable(caption="Not by patient, by positive swab status")

ungroup(population_rates) %>% kable(caption="Positive swab proportion by patient. (population = pooled proportion).")

```


```{r episode infection breastfeeding comparison}
...
ungroup(all_swab_rates) %>% kable(caption="Positive swab proportion by all swabs. (population = pooled proportion).")
